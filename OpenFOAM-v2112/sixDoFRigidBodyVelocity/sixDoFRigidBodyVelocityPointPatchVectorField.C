...
#include "sixDoFRigidBodyVelocityPointPatchVectorField.H"
...
sixDoFRigidBodyVelocityPointPatchVectorField
(
    const sixDoFRigidBodyVelocityPointPatchVectorField& ptf,
    const pointPatch& p,
    const DimensionedField<vector, pointMesh>& iF,
...
void sixDoFRigidBodyVelocityPointPatchVectorField::autoMap
(
...
void sixDoFRigidBodyVelocityPointPatchVectorField::rmap
(
	const pointPatchField<vector> & ptf,
    const labelList& addr
)
{
    const sixDoFRigidBodyVelocityPointPatchVectorField& sDoFptf =
        refCast<const sixDoFRigidBodyVelocityPointPatchVectorField>(ptf);
...
void sixDoFRigidBodyVelocityPointPatchVectorField::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    pointField velocities(initialPoints_.size());
    List<label> pId;
    forAll(initialPoints_, pId)
    {
        vector velocity = motion_.velocity(motion_.transform(initialPoints_[pId]));
    ...
    (
        velocities
        //motion_.velocity(motion_.transform(initialPoints_)())
    );
...
void sixDoFRigidBodyVelocityPointPatchVectorField::write(Ostream& os) const
{
